...
export function ActivityTabs({
  ...
  function holderLabel(address: Address): string | undefined {
    const a = address.toLowerCase();
    if (pool !== ZERO_ADDRESS && a === pool.toLowerCase()) return "Uniswap V3 Pool";
        ...
        <div className="mt-3">
          <ul className="divide-y divide-neutral-800/70">
            {topHolders.map((h, i) => {
              ...
              return (
                <li key={h.address} className="flex items-center gap-3 py-2.5 text-sm">
                  <span className="w-5 shrink-0 font-mono text-xs text-neutral-600">
                    {i + 1}
                  </span>
                  <span className="min-w-0 flex-1 truncate">
                    ...
                    {label && (
                      <span className="ml-2 rounded bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-semibold text-amber-400">
                        {label}
                      </span>
                    )}
                  </span>
                  ...
                  <span className="w-16 shrink-0 text-right font-mono text-xs text-neutral-200">
                    {pct}%
                  </span>
                </li>
              );
